...
int main(int argc, char *argv[]){
    ...
    dim3 dimGrid    = dim3(BLOCK_WIDTH, BLOCK_WIDTH, 1);
    ...
    char str[128];
    if (bAccuracy == false){
        sprintf(str, "Comparison of CPU and GPU Matrix Multiplication is not accurate at array index %d %d, %d", breakValue, gold[breakValue], hostC[breakValue]);
    }
    else{
