...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow){
    ...
    hWnd = CreateWindowEx(WS_EX_APPWINDOW, szAppName, TEXT("RTR 6 - Akash Musale - Tesselation Shader"), WS_OVERLAPPEDWINDOW | WS_CLIPCHILDREN | WS_CLIPSIBLINGS | WS_VISIBLE, (screenWidth - WIN_WIDTH) / 2, (screenHeight  - WIN_HEIGHT) / 2, WIN_WIDTH, WIN_HEIGHT, NULL, NULL, hInstance, NULL);
    ghWnd = hWnd;
...
int initialize(void){
    ...
    */
    ...
    iStatus = 0;
    szInfoLog = NULL;
    glGetShaderiv(tesselationControlShaderObject, GL_COMPILE_STATUS, &iStatus);
    if(iStatus == GL_FALSE){
        glGetShaderiv(tesselationControlShaderObject, GL_INFO_LOG_LENGTH, &iInfoLogLength);
        if(iInfoLogLength > 0){
            szInfoLog = (GLchar *)malloc(iInfoLogLength * sizeof(GLchar));
            if(szInfoLog != NULL){
                glGetShaderInfoLog(tesselationControlShaderObject, iInfoLogLength, NULL, szInfoLog);
                fprintf(gpFile, "tesselationControl Shader Compilation Log: %s\n", szInfoLog);
                free(szInfoLog);
                szInfoLog = NULL;
            }
        }
    ...
    iStatus = 0;
    szInfoLog = NULL;
    glGetShaderiv(tesselationEvaluationShaderObject, GL_COMPILE_STATUS, &iStatus);
    if(iStatus == GL_FALSE){
        glGetShaderiv(tesselationEvaluationShaderObject, GL_INFO_LOG_LENGTH, &iInfoLogLength);
        if(iInfoLogLength > 0){
            szInfoLog = (GLchar *)malloc(iInfoLogLength * sizeof(GLchar));
            if(szInfoLog != NULL){
                glGetShaderInfoLog(tesselationEvaluationShaderObject, iInfoLogLength, NULL, szInfoLog);
                fprintf(gpFile, "tesselationEvaluation Shader Compilation Log: %s\n", szInfoLog);
                free(szInfoLog);
                szInfoLog = NULL;
            }
        }
...
void display(void){
    ...
    glUseProgram(shaderProgramObject);
    {
        ...
        mat4 modelViewProjectionMatrix = mat4::identity();
        {
            ...
            TCHAR str[255];
            wsprintf(str, TEXT("RTR 6 - Akash Musale - Tesselation Shader : No Of Line Segments = %d"), uiNoOfLineSegments);
