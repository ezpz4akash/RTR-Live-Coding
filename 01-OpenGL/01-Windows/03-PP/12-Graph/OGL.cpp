...
GLuint vao_AxisLines = 0;
GLuint vbo_position_AxisLines = 0;
GLuint vbo_position_AxisColors = 0;

GLuint vao_thinBlueLine = 0;
GLuint vbo_position_thinBlueLines = 0;

GLuint vao_thickBlueLine = 0;
GLuint vbo_position_thickBlueLines = 0;
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow){
    ...
    hWnd = CreateWindowEx(WS_EX_APPWINDOW, szAppName, TEXT("RTR 6 - Akash Musale - Graph Using PP"), WS_OVERLAPPEDWINDOW | WS_CLIPCHILDREN | WS_CLIPSIBLINGS | WS_VISIBLE, (screenWidth - WIN_WIDTH) / 2, (screenHeight  - WIN_HEIGHT) / 2, WIN_WIDTH, WIN_HEIGHT, NULL, NULL, hInstance, NULL);
    ghWnd = hWnd;
    ...
    */
    ...
    //VAO Setup Thin Blue Lines
    {
        ...
        glBindVertexArray(vao_thinBlueLine);
        {
            ...
            // Color VBO
            // For color we will be using glVertexAttrib4fv as its a constant color for each primitive
        }
    ...
    //VAO Setup Thick Blue Lines
    {
        ...
        for(int i = 0; i < 8 * 3 * 2 * 2; i = i + 6){
            fprintf(gpFile, "Line from : %f, %f, %f TO %f, %f, %f\n", thickBlueLinePosition[i], thickBlueLinePosition[i + 1], thickBlueLinePosition[i + 2], thickBlueLinePosition[i + 3], thickBlueLinePosition[i + 4], thickBlueLinePosition[i + 5]);
        }
    }
    ...
    //VAO Setup Axis Lines
    {
        ...
        glBindVertexArray(vao_AxisLines);
        {
            ...
            // Color VBO
            {
                glGenBuffers(1, &vbo_position_AxisColors);
                glBindBuffer(GL_ARRAY_BUFFER, vbo_position_AxisColors);
                {
    ...
    //VAO Setup Circle
    {
        ...
        #define NO_OF_FLOATS_IN_3D 3
        #define GL_PI 3.145
        #define DEG_TO_RAD(deg) ((GLfloat)deg * (GL_PI / 180.0f))
        GLfloat circlePosition[NO_OF_CIRCLE_POINTS * NO_OF_FLOATS_IN_3D] = {};
        GLfloat radius = 0.5f;
        GLint i = 0;
        for(GLfloat angle = 0.0f; angle < (360.0f); angle = angle + 1.0f, i = i + 3){
            circlePosition[i] = (radius * cos(DEG_TO_RAD(angle)));
            circlePosition[i + 1] = (radius * sin(DEG_TO_RAD(angle)));
            circlePosition[i + 2] = 0.0f;
        }
        i = 0;
        for(i = 0; i < NO_OF_CIRCLE_POINTS * NO_OF_FLOATS_IN_3D; i = i + 3){
            fprintf(gpFile, "Circle Point %d : %f, %f, %f\n", (i / 3) + 1, circlePosition[i], circlePosition[i + 1], circlePosition[i + 2]);
        }
        // Create Vertex Array Object (VAO) for array of vertex attributes
        // vao is an array object that stores the state of vertex attributes
        ...
        mat4 translationMatrix = mat4::identity();
        {
            ...
            if(drawGraph){
                ...
                glBindVertexArray(vao_thinBlueLine);
                {
                    // Draw the triangle
                    glLineWidth(1.0f);
                    glVertexAttrib4f(AMC_ATTRIBUTE_COLOR, 0.0f, 0.0f, 1.0f, 1.0f);

                    glDrawArrays(GL_LINES, 0, 32 * 2 * 2);
                }
                glBindVertexArray(0);
            ...
            if(drawTriangle){
                ...
                glBindVertexArray(vao_triangle);
                {
                    ...
                    glVertexAttrib4f(AMC_ATTRIBUTE_COLOR, 1.0f, 1.0f, 0.0f, 1.0f);
