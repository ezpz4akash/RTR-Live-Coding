...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow){
    ...
    hWnd = CreateWindowEx(WS_EX_APPWINDOW, szAppName, TEXT("RTR 6 - Akash Musale - Lights - SpecularLightPerVertex"), WS_OVERLAPPEDWINDOW | WS_CLIPCHILDREN | WS_CLIPSIBLINGS | WS_VISIBLE, (screenWidth - WIN_WIDTH) / 2, (screenHeight  - WIN_HEIGHT) / 2, WIN_WIDTH, WIN_HEIGHT, NULL, NULL, hInstance, NULL);
    ghWnd = hWnd;
...
int initialize(void){
    ...
    */
    ...
    const GLchar *vertexShaderSourceCode = 
        ...
        "       vec3 reflectedLight = normalize(reflect(-lightSource, transformedNormal));\n" \
        "       vec3 viewVector = normalize(-eyeCoordinates.xyz);\n" \
        "       float rDotv = max(dot(reflectedLight, viewVector), 0.0);\n" \
        "       vec4 specularLight = vec4(uLs * uKs * pow(rDotv, 10.0), 1.0);\n" \
        "       out_diffuseLight = vec4(uLd * uKd * tnDotLd, 1.0) + specularLight;\n" \
        "   }\n" \
        "   else\n" \
        "   {\n" \
