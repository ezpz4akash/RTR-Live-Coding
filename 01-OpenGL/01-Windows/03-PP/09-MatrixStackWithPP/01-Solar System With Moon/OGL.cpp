...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow){
    ...
    hWnd = CreateWindowEx(WS_EX_APPWINDOW, szAppName, TEXT("RTR 6 - Akash Musale - MatrixStackPP - SolarSystem"), WS_OVERLAPPEDWINDOW | WS_CLIPCHILDREN | WS_CLIPSIBLINGS | WS_VISIBLE, (screenWidth - WIN_WIDTH) / 2, (screenHeight  - WIN_HEIGHT) / 2, WIN_WIDTH, WIN_HEIGHT, NULL, NULL, hInstance, NULL);
    ghWnd = hWnd;
...
void display(void){
    ...
    glUseProgram(shaderProgramObject);
    {
        ...
        transformationMatrixStack.push(modelViewMatrix);
        {
            ...
            modelViewMatrix = modelViewMatrix * rotationMatrix * translationMatrix * vmath::rotate(date, 0.0f, 1.0f, 0.0f) * scaleMatrix;

            modelViewProjectionMatrix = perspectiveProjectionMatrix * modelViewMatrix;
        
            glVertexAttrib4f(AMC_ATTRIBUTE_COLOR, 0.4f, 0.7f, 1.0f, 1.0f);
            glUniformMatrix4fv(mvpMatrixUniform, 1, GL_FALSE, modelViewProjectionMatrix);
            ...
            transformationMatrixStack.push(modelViewMatrix);
            {
                ...
                rotationMatrix = vmath::rotate(date, 0.0f, 1.0f, 0.0f);
                translationMatrix = vmath::translate(1.5f, 0.0f, 0.0f);
                modelViewMatrix = modelViewMatrix * rotationMatrix * translationMatrix * vmath::rotate(moon, 0.0f, 1.0f, 0.0f) * scaleMatrix;
                glVertexAttrib4f(AMC_ATTRIBUTE_COLOR, 0.5f, 0.5f, 0.5f, 1.0f);
                modelViewProjectionMatrix = perspectiveProjectionMatrix * modelViewMatrix;
                glUniformMatrix4fv(mvpMatrixUniform, 1, GL_FALSE, modelViewProjectionMatrix);
                glPolygonMode(GL_FRONT_AND_BACK, GL_LINE);
                glBindVertexArray(gVao_sphere);
