...
// OpenCL related header file
#include <CL/opencl.h>

#pragma comment(lib, "OpenCL.lib")    // OpenCL
...
int initialize(void){
    ...
    if(oclResult != CL_SUCCESS){
        ...
        fprintf(gpFile, "There is no OpenCL supported device on this system\n");
        uninitialize();
        exit(EXIT_FAILURE);
    }
    ...
    // Initialize OpenCL Context Properties Array
    cl_context_properties oclContextProperties[] = {
        CL_GL_CONTEXT_KHR, (cl_context_properties)wglGetCurrentContext(), //glxGetCurrentContext()
        CL_WGL_HDC_KHR, (cl_context_properties)wglGetCurrentDC(), //CL_GLX_DISPLAY_KHR, glxGetCurrentDisplay()
    ...
    if(oclResult != CL_SUCCESS){
        size_t len;
        char szBuffer[2048];
        clGetProgramBuildInfo(oclProgram, oclDeviceID, CL_PROGRAM_BUILD_LOG, sizeof(szBuffer), szBuffer, &len);
        ...
        mat4 rotationMatrix = mat4::identity();
        {
            ...
            glBindVertexArray(vao);
            {
                if(onGPU == TRUE){
                    ...
                    if(oclResult != CL_SUCCESS){
                        fprintf(gpFile, "clSetKernelArg() failed : %d\n", oclResult);
                        uninitialize();
                        exit(EXIT_FAILURE);
                    }
