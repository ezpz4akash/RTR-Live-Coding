...
// CUDA related headers and lobraries
#include <cuda_gl_interop.h>
#include <cuda_runtime.h>

#include "Sinewave.h"

#pragma comment(lib, "cudart.lib") // Import library
...
//color related variable snad macros
#define RED_CODE     0
#define GREEN_CODE   1
#define BLUE_CODE    2
...
int initialise(void)
{
    ...
    {
        fprintf(gpFile, "cudaGetDeviceCount() no CUDA supported devices found");
        uninitialise();
        exit(EXIT_FAILURE);
    }
...
void display(void)
{
    ...
    if (bOnGPU == TRUE)
    {
        ...
        //get host compatible mapped pointer for CUDA graphics resource
        ...
        sineWaveKernelStart(pPosition, gMeshWidth, gMeshHeight, animationTime);

        //unmap the mapped pointer 
        cudaResult = cudaGraphicsUnmapResources(1, &cuGraphicsResource, 0);
        //remembe tha tout pPosition is vbo_gpu hence biand with it
        glBindBuffer(GL_ARRAY_BUFFER, vbo_gpu);
    ...
    else
    {
        ...
        fprintf(gpFile, "GL_BUFFER_SIZE = %d (expected %zu)\n", bufSize, (size_t)gMeshWidth * gMeshHeight * 4 * sizeof(float));
    }
