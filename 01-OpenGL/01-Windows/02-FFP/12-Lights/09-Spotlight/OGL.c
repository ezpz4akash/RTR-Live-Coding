...
GLfloat lightPosition[] = { 0.0f, 2.5f, -7.5f, 1.0f };
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow){
    ...
    hWnd = CreateWindowEx(WS_EX_APPWINDOW, szAppName, TEXT("RTR 6 - Akash Musale - Spotlight"), WS_OVERLAPPEDWINDOW | WS_CLIPCHILDREN | WS_CLIPSIBLINGS | WS_VISIBLE, (screenWidth - WIN_WIDTH) / 2, (screenHeight  - WIN_HEIGHT) / 2, WIN_WIDTH, WIN_HEIGHT, NULL, NULL, hInstance, NULL);
    ghWnd = hWnd;
...
int initialize(void){
    ...
    // This will allow us to use glColor3f() to set the color of the material
...
void display(void){
	...
	glBegin(GL_QUADS);
	for (GLfloat x = -3.0f; x < 3.0f; x = x + 0.01f)
	{
		for (GLfloat z = 0.0f; z > -5.0f; z = z - 0.01f)
		{
