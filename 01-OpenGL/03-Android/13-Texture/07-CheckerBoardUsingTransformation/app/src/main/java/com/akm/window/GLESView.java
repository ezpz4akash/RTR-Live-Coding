...
public class GLESView extends GLSurfaceView implements GLSurfaceView.Renderer, OnGestureListener, OnDoubleTapListener
{
    ...
    //Texture variables
    private int textureCheckerBoard;
    private int textureSamplerUniform;

    //Checkerboard texture variables
    private static final int checkerImageWidth = 64;
    private static final int checkerImageHeight = 64;
    ...
    private int initialize(GL10 gl)
    {
        ...
        GLES32.glClearColor(0.75f, 0.75f, 0.75f, 1.0f); //Blue Screen

        textureCheckerBoard = loadGLTexture();
    ...
    private int loadGLTexture(){
        ...
        //Create bitmap image from byte buffer
    ...
    private byte[] makeCheckerBoard()
    {
        ...
        for(i = 0; i < checkerImageHeight; i++)
        {
            for(j = 0; j < checkerImageWidth; j++)
            {
                ...
                checkerImage[(i * checkerImageWidth + j) * 4 + 0] = (byte)c; //Red
                checkerImage[(i * checkerImageWidth + j) * 4 + 1] = (byte)c; //Green
                checkerImage[(i * checkerImageWidth + j) * 4 + 2] = (byte)c; //Blue
                checkerImage[(i * checkerImageWidth + j) * 4 + 3] = (byte)255; //Alpha
            }
        }
        return checkerImage;
    }
    ...
    private void display()
    {
        ...
        GLES32.glUseProgram(shaderProgramObject);
        {
            ...
            //Transformations
            Matrix.translateM(translationMatrix, 0, -1.0f, 0.0f, -5.0f);
            Matrix.multiplyMM(modelViewMatrix, 0, modelViewMatrix, 0, translationMatrix, 0);
            Matrix.multiplyMM(modelViewProjectionMatrix, 0, perspectiveProjectionMatrix, 0, modelViewMatrix, 0);

            GLES32.glUniformMatrix4fv(mvpMatrixUniform, 1, false, modelViewProjectionMatrix, 0);

            //Bind with checkerboard texture
            GLES32.glActiveTexture(GLES32.GL_TEXTURE0);
            GLES32.glBindTexture(GLES32.GL_TEXTURE_2D, textureCheckerBoard);
            ...
            Matrix.translateM(translationMatrix, 0, 1.0f, 0.0f, -5.5f);
            Matrix.rotateM(rotationMatrix, 0, 40.0f, 0.0f, 1.0f, 0.0f);
            Matrix.multiplyMM(modelViewMatrix, 0, modelViewMatrix, 0, translationMatrix, 0);
    ...
    private void uninitialize()
    {
        ...
        if(textureCheckerBoard != 0)
        {
            GLES32.glDeleteTextures(1, new int[] {textureCheckerBoard}, 0);
