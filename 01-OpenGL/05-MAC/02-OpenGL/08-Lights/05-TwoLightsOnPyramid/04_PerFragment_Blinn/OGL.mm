...
-(int) initialize
{
   ...
   // Step 1 Write the shader source code
    ...
    "       fong_blinn_ads_light = vec3(0.0f, 0.0f, 0.0f);\n" \
    "       for (int light_no = 0; light_no < 2; light_no++)\n" \
    "       {\n" \
    "           vec3 normalized_lightDirection = normalize(out_lightDirection[light_no]);\n"
    "           vec3 halfVector = normalize(normalized_lightDirection + normalized_viewerVector);\n"
    ...
    "           vec3 specularLight = uLs[light_no] * uKs * pow(max(dot(halfVector, normalized_transformedNormals),0.0), 2);\n"
    "           fong_blinn_ads_light += ambientLight + diffusedLight + specularLight;\n"
    "       }\n" \
    "   }\n" \
    "   else \n"\
    "   {\n"\
    "       fong_blinn_ads_light = vec3(1.0f, 1.0f, 1.0f);\n"
    "   }\n" \
    "   FragColor = vec4(fong_blinn_ads_light, 1.0f);\n" \
    "}\n";
...
-(void)display
{
    ...
    static GLfloat materialShininess = 50.0f;
    //code
