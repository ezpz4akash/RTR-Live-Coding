...
-(int) initialize
{
   ...
   // Step 1 Write the shader source code
    ...
    "       out_blinn_ads_light = vec3(0.0f, 0.0f, 0.0f);\n" \
    ...
    "           halfVector[light_no] = normalize(lightDirection[light_no] + viewerVector) / length(lightDirection[light_no] + viewerVector);\n"
    "           specularLight[light_no] = uLs[light_no]  * uKs * pow(max(dot(halfVector[light_no], viewerVector),0.0), 2);\n"
    "           out_blinn_ads_light += ambientLight[light_no] + diffusedLight[light_no] + specularLight[light_no];\n"
    "       }\n"\
    "   }\n"\
    "   else \n"\
    "   {\n"\
    "       out_blinn_ads_light = vec3(1.0f,1.0f,1.0f);\n"
    "   }\n"\
    "}\n";
    ...
    "#version 410 core\n" \
    "out vec4 FragColor;\n" \
    "in vec3 out_blinn_ads_light;\n" \
    "void main(void)" \
    "{\n" \
    "   FragColor = vec4(out_blinn_ads_light, 1.0f);\n" \
    "}\n";
