...
-(int) initialize
{
   ...
   // Step 1 Write the shader source code
    ...
    const GLfloat pyramid_texcords[] = {
        // front
        0.5, 1.0, // front-top
        0.0, 0.0, // front-left
        1.0, 0.0, // front-right
 ...
 // vertex array object for arrays of vertex attributes
    ...
    glBindVertexArray(vao_pyramid);
        ...
        //texcords
        // Step 1. craete buffer in GPU memory
        glGenBuffers(1, &vbo_texcords_pyramid);

        // Step2. bind the buffer
        glBindBuffer(GL_ARRAY_BUFFER, vbo_texcords_pyramid);

        //step3
        glBufferData(GL_ARRAY_BUFFER, sizeof(pyramid_texcords), pyramid_texcords, GL_STATIC_DRAW);
    ...
    textureStone = [self loadGLTexture:"Stone.bmp"];
    textureKundali = [self loadGLTexture:"Kundali.bmp"];
...
-(void)uninitialize
{
    ...
    if (vbo_texcords_pyramid)
    {
        glDeleteBuffers(1, &vbo_texcords_pyramid);
        vbo_texcords_pyramid = 0;
    }
