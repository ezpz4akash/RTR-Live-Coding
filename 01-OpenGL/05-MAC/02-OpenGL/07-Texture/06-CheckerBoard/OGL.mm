...
-(int) initialize
{
   ...
   // Step 1 Write the shader source code
    ...
    const GLfloat rectangle_texcords[] = {
        // front
        1.0f, 1.0f, // right-top
        0.0f, 1.0f, // left-top
    ...
    glBindVertexArray(vao_rectangle);
        ...
        //step3
        glBufferData(GL_ARRAY_BUFFER, sizeof(rectangle_texcords), rectangle_texcords, GL_STATIC_DRAW);
...
{    // function declaration
    ...
    // dont do unpacking(default unpacking)
    ...
    // 1, binding point
    // 2, Image format no of components (3 colors)
    // 3. width
    // 4. height
...
-(void)makeCheckerboard
{
    ...
    //code
    for( height = 0; height < CHECKERBOARD_HEIGHT; height++)
    {
        for( width = 0; width < CHECKERBOARD_WIDTH; width++) 
        {
            c = (((height & 0x08) == 0) ^ ((width & 0x08) == 0)) * 255;
            checkerboard[height][width][0] = (GLubyte)c;
            checkerboard[height][width][1] = (GLubyte)c;
            checkerboard[height][width][2] = (GLubyte)c;
            checkerboard[height][width][3] = (GLubyte)255;
        }
    }
}
