...
-(int) initialize
{
   ...
   // Step 1 Write the shader source code
    ...
    glBindVertexArray(vao_rectangle);
        ...
        //texcords
        // Step 1. craete buffer in GPU memory
        glGenBuffers(1, &vbo_texcords);

        // Step2. bind the buffer
        glBindBuffer(GL_ARRAY_BUFFER, vbo_texcords);

        //step3
        //sizeof(float * 4 * 2) - 4 texcords, 2- u,v
    ...
    textureSmiely = [self loadGLTexture:"Smiley.bmp"];
...
- (GLuint)loadGLTexture:(const char *)imageFileName {
  ...
  if (!bmpImage) {
    fprintf(gpFile, "%s texture not found..\n", imageFileName);
    return (0);
  }
...
-(void)display
{
    ...
    glBindTexture(GL_TEXTURE_2D, textureSmiely);
    // parameter 2. index of the sampler
    // parameter 3. 
    glUniform1i(textureSamplerUniform, 0);

    glUniform1i(keyPressUniform, showSmiley);

     GLfloat rectangle_textcords[8] = {0};
    if (showSmiley == 1)
    {
        rectangle_textcords[0] = 0.5f;
        rectangle_textcords[1] = 0.5f;
        rectangle_textcords[2] = 0.0f;
        rectangle_textcords[3] = 0.5f;
        rectangle_textcords[4] = 0.0f;
        rectangle_textcords[5] = 0.0f;
        rectangle_textcords[6] = 0.5f;
        rectangle_textcords[7] = 0.0f;
    }
    else if (showSmiley == 2)
    {
        rectangle_textcords[0] = 1.0f;
        rectangle_textcords[1] = 1.0f;
        rectangle_textcords[2] = 0.0f;
        rectangle_textcords[3] = 1.0f;
        rectangle_textcords[4] = 0.0f;
        rectangle_textcords[5] = 0.0f;
        rectangle_textcords[6] = 1.0f;
        rectangle_textcords[7] = 0.0f;
    }
    else if (showSmiley == 3)
    {
        rectangle_textcords[0] = 2.0f;
        rectangle_textcords[1] = 2.0f;
        rectangle_textcords[2] = 0.0f;
        rectangle_textcords[3] = 2.0f;
        rectangle_textcords[4] = 0.0f;
        rectangle_textcords[5] = 0.0f;
        rectangle_textcords[6] = 2.0f;
        rectangle_textcords[7] = 0.0f;
    }
    else if (showSmiley == 4)
    {
        rectangle_textcords[0] = 0.51f;
        rectangle_textcords[1] = 0.51f;
        rectangle_textcords[2] = 0.49f;
        rectangle_textcords[3] = 0.51f;
        rectangle_textcords[4] = 0.49f;
        rectangle_textcords[5] = 0.49f;
        rectangle_textcords[6] = 0.51f;
        rectangle_textcords[7] = 0.49f;
    }
    else
    {
    ...
    glBindBuffer(GL_ARRAY_BUFFER, vbo_texcords);
    glBufferData(GL_ARRAY_BUFFER, sizeof(rectangle_textcords), rectangle_textcords, GL_DYNAMIC_DRAW);
...
-(void)uninitialize
{
    ...
    if (textureSmiely)
    {
        glDeleteTextures(1, &textureSmiely);
        textureSmiely = 0;
    }
    ...
    if (vbo_texcords)
    {
        glDeleteBuffers(1, &vbo_texcords);
        vbo_texcords = 0;
    }
