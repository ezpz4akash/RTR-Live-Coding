...
GLfloat materiaShininess= 128.0f;
...
-(int) initialize
{
   ...
   // Step 1 Write the shader source code
    ...
    "       final_light = vec4(fong_ads_light, 1.0f) * out_color * texture(uTextureSampler, out_texcord);\n" \
    "   }\n" \
    "   else \n"\
    "   {\n"\
    "       fong_ads_light = vec3(1.0f, 1.0f, 1.0f);\n"
    "       final_light = vec4(fong_ads_light, 1.0);\n"
    "   }\n" \
    ...
    glBindVertexArray(vao_cube);
        ...
        //for textcords
        glVertexAttribPointer(AMC_ATTRIBUTE_TEXCORDS, 2, GL_FLOAT, GL_FALSE, 11*sizeof(float), (void *)(9*sizeof(float)));
    ...
    // load textures
    textureMarble = [self loadGLTexture:"marble.png"];
...
-(void)display
{
 ...
 //use shader program object
    ...
    modelMatrix = translationMatrix * rotationMatrixX * rotationMatrixY *rotationMatrixZ * scaleMatrix; // order is important
