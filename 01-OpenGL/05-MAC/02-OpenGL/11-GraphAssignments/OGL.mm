...
#define SHOW_GRAPH     (0x08)
#define CIRCLE_POINTS  (1000)
#define GRAPH_NO_LINES (41)
...
@implementation GLView
{
    ...
    unsigned int gVisibilityControl;
}
...
-(int) initialize
{
   ...
   // Step 1 Write the shader source code
    ...
    // we need 3 lines for trangle , so 3 points if we use line_loop premitive
    GLfloat triangle_position[3][3];
    ...
    for (int index = 0; index < 3; index ++)
    {
        ...
        triangle_position[index][0] = cos(angle);
        triangle_position[index][1] = sin(angle);
        triangle_position[index][2] = 0.0f;
    }
   ...
   // vertex array object for arrays of vertex attributes
    ...
    // circle related buffers

    GLfloat circle_position[CIRCLE_POINTS][3];

    float angle = 0.0f;

    for (int index = 0; index < CIRCLE_POINTS; index ++)
    {

        angle += (2 * M_PI) / CIRCLE_POINTS;
        circle_position[index][0] = cos(angle);
        circle_position[index][1] = sin(angle);
        circle_position[index][2] = 0.0f;
    }
    ...
    // graph paper related buffers
    //We need 41 horizontal and 41 vertical lines
    // out of these 41, 9 horizontal and 9 veritical are thick so lets repeat then at the end of buffer
     // out of these 41, 1 horizontal and 1 vertical at center is color full 

    GLfloat graph_position[GRAPH_NO_LINES + 10][2][2][3];

    float x = 1.0f;
    float y = 1.0f;
    float gap = 2.0f/40.0f;
    for (int line = 0; line < GRAPH_NO_LINES; line++)
    {   
        //horizontal line point 1
        graph_position[line][0][0][0] = -1.0f; //x
        graph_position[line][0][0][1] = y;     //y
        graph_position[line][0][0][2] = 0.0f;  //z

        //horizontal line point 2
        graph_position[line][0][1][0] = 1.0f; //x
        graph_position[line][0][1][1] = y;     //y
        graph_position[line][0][1][2] = 0.0f;  //z

        //vertical line point 1
        graph_position[line][1][0][0] = x; //x
        graph_position[line][1][0][1] = -1.0;     //y
        graph_position[line][1][0][2] = 0.0;  //z

        //vertical line point 2
        graph_position[line][1][1][0] = x; //x
        graph_position[line][1][1][1] = 1.0;     //y
        graph_position[line][1][1][2] = 0.0f;  //z

        x = x - gap; 
        y = y - gap;  
    }

    //lines to show in bold
    for (int line = 0; line < 9; line++)
    {   
        //copy each fifth line
        int src = line * 5; // every 5th (0,5,10,...)
        int dst = GRAPH_NO_LINES + line;
        memcpy(&graph_position[dst], &graph_position[src], sizeof(graph_position[0]));
    }

    //finally center lines
    memcpy(&graph_position[GRAPH_NO_LINES + 9], &graph_position[20], sizeof(graph_position[0]));
    ...
    glBindVertexArray(vao_graph);
        ...
        //position
        // Step 1. craete buffer in GPU memory
        glGenBuffers(1, &vbo_position_graph);

        // Step2. bind the buffer
        glBindBuffer(GL_ARRAY_BUFFER, vbo_position_graph);
...
-(void)display
{
    ...
    if (gVisibilityControl & SHOW_GRAPH)
    {
        ...
        glLineWidth(1.0f);
        glDrawArrays(GL_LINES, 0, GRAPH_NO_LINES*2*2);
        glLineWidth(3.0f);
        glDrawArrays(GL_LINES, GRAPH_NO_LINES*2*2, 9*2*2);
        glLineWidth(4.0f);
        glUniform4f(colorUniform, 1.0f, 0.0f, 0.0f, 1.0f);
        glDrawArrays(GL_LINES, (GRAPH_NO_LINES + 9)*2*2, 2);
        glUniform4f(colorUniform, 0.0f, 1.0f, 0.0f, 1.0f);
        glDrawArrays(GL_LINES, (GRAPH_NO_LINES + 9)*2*2 + 2, 2);
    ...
    translationMatrix = vmath::translate(0.0f, 0.0f, -3.0f);
    scaleMatrix = vmath::scale(0.7f, 0.7f, 1.0f);
...
-(void)uninitialize
{
    ...
    if (vbo_position_graph)
    {
        glDeleteBuffers(1, &vbo_position_graph);
        vbo_position_graph = 0;
    }
    if (vao_graph)
    {
