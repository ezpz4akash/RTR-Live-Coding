...
@implementation GLView
{
    ...
    GLuint numOfLineSegmentsUniform;
    GLuint numOfLineStripsUniform;
    GLuint lineColorUniform;
    unsigned int uiNumOfLineSegments;
...
-(void)keyDown:(NSEvent *)event
{
    ...
    switch(key)
    {
        ...
        case NSUpArrowFunctionKey:
            uiNumOfLineSegments++;
            if (uiNumOfLineSegments >= 30)
            {
                uiNumOfLineSegments = 30;
            }
            break;

        case NSDownArrowFunctionKey:
            if (uiNumOfLineSegments > 1)
            {
                uiNumOfLineSegments--;
            }
            break;
...
-(int) initialize
{
   ...
   // Step 1 Write the shader source code
    ...
    // Attach TCS to the shader program object
    glAttachShader(shaderProgramObject, tessellationControlShaderObject);

    // Attach TES to the shader program object
    ...
    numOfLineSegmentsUniform = glGetUniformLocation(shaderProgramObject, "uNumberOfLineSegments");
    numOfLineStripsUniform = glGetUniformLocation(shaderProgramObject, "uNumberOfLineStrips");
...
-(void)display
{
    ...
    glUniform1i(numOfLineSegmentsUniform, uiNumOfLineSegments);
    glUniform1i(numOfLineStripsUniform, 1);
