...
-(void)awakeFromNib
{
    [super awakeFromNib];
    {
        ...
        lightPosition[0] = 0.0f; lightPosition[1] = 0.0f; lightPosition[2] = 2.0f;
        bLight = YES;
        bAnimate = YES;
...
-(int)initialize
{
    ...
    // Vertex Shader (diffuse lighting)
    ...
    const GLchar *vertexShaderSourceCode =
        ...
        "        out_diffusedLight = uLd * uKd * max(dot(lightSource, transformedNormal), 0.0);\n"
        "    }\n"
        "    else\n"
        "    {\n"
        "        out_diffusedLight = vec3(1.0, 1.0, 1.0);\n"
        "    }\n"
        "}\n";
    ...
    const GLchar *fragmentShaderSourceCode =
        ...
        "in vec3 out_diffusedLight;\n"
        "out vec4 FragColor;\n"
        "void main(void)\n"
        "{\n"
        "    FragColor = vec4(out_diffusedLight, 1.0);\n"
        "}\n";
...
-(void)display
{
    ...
    // rotate the model (only if animation is enabled)
    if(bAnimate)
    {
        float radians = angle_sphere * 3.14159f / 180.0f;
        modelViewMatrix = vmath::rotate(radians, 0.0f, 1.0f, 0.0f) * modelViewMatrix;
    }
    ...
    glUniform4f(lightPositionUniform, lightPosition[0], lightPosition[1], lightPosition[2], 0.0f);
