...
-(int)initialize
{
    ...
    // Vertex Shader - Per Vertex Blinn-Phong with 2 lights
    ...
    const GLchar *vertexShaderSourceCode =
        ...
        "           vec3 halfVector = normalize(lightSource + viewerVector);\n"
        "           vec3 ambient = uLa[i] * uKa;\n"
        "           vec3 diffuse = uLd[i] * uKd * tnDotLd;\n"
        "           vec3 specular = uLs[i] * uKs * pow(max(dot(halfVector, transformedNormal), 0.0), uMaterialShininess);\n"
