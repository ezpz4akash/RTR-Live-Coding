...
-(int)initializeSphere
{
    ...
    const GLchar *pv_vertexShaderSourceCode =
        ...
        "       for(int i = 0; i < 3; i++)\n"
        "       {\n"
        "           vec3 lightSource = normalize(vec3(uLightPosition[i]) - eyeCoordinates.xyz);\n"
        ...
        "           vec3 specular = uLs[i] * uKs * pow(max(dot(reflectedVector, viewerVector), 0.0), uMaterialShininess);\n"
        "           out_phong_ads_Light = out_phong_ads_Light + vec4(ambient + diffuse + specular, 0.0);\n"
        "       }\n"
        "   }\n"
        "   else\n"
        "   {\n"
    ...
    for(int i = 0; i < NO_OF_LIGHTS; i++){
        char buffer[64];
        sprintf(buffer, "uLa[%d]", i);
        pv_laUniform_sphere[i] = glGetUniformLocation(pv_shaderProgramObject_sphere, buffer);
        sprintf(buffer, "uLd[%d]", i);
        pv_ldUniform_sphere[i] = glGetUniformLocation(pv_shaderProgramObject_sphere, buffer);
        sprintf(buffer, "uLs[%d]", i);
        pv_lsUniform_sphere[i] = glGetUniformLocation(pv_shaderProgramObject_sphere, buffer);
        sprintf(buffer, "uLightPosition[%d]", i);
        pv_lightPositionUniform_sphere[i] = glGetUniformLocation(pv_shaderProgramObject_sphere, buffer);
    }
    ...
    const GLchar *pf_fragmentShaderSourceCode =
        ...
        "void main(void)\n"
        "{\n"
        "   vec4 out_phong_ads_Light = vec4(0.0, 0.0, 0.0, 1.0);\n"
        ...
        "           vec3 ambient = uLa[i] * uKa;\n"
        "           vec3 diffuse = uLd[i] * uKd * tnDotLd;\n"
        "           vec3 specular = uLs[i] * uKs * pow(max(dot(reflectedVector, normalizedViewerVector), 0.0), uMaterialShininess);\n"
        "           out_phong_ads_Light = out_phong_ads_Light + vec4(ambient + diffuse + specular, 0.0);\n"
        "       }\n"
        "       FragColor = out_phong_ads_Light;\n"
        "   }\n"
    ...
    for(int i = 0; i < NO_OF_LIGHTS; i++){
        char buffer[64];
        sprintf(buffer, "uLa[%d]", i);
        pf_laUniform_sphere[i] = glGetUniformLocation(pf_shaderProgramObject_sphere, buffer);
        sprintf(buffer, "uLd[%d]", i);
        pf_ldUniform_sphere[i] = glGetUniformLocation(pf_shaderProgramObject_sphere, buffer);
        sprintf(buffer, "uLs[%d]", i);
        pf_lsUniform_sphere[i] = glGetUniformLocation(pf_shaderProgramObject_sphere, buffer);
        sprintf(buffer, "uLightPosition[%d]", i);
        pf_lightPositionUniform_sphere[i] = glGetUniformLocation(pf_shaderProgramObject_sphere, buffer);
    }
...
-(void)displaySphere
{
    ...
    if(bPerVertexPerFragmentToggle){
        ...
        // Light 0 - Red - rotates around Y axis
        {
            float radians = lightAngle[0] * M_PI / 180.0f;
            float lightPos[4] = { 20.0f * sinf(radians), 0.0f, 20.0f * cosf(radians), 1.0f };
        ...
        // Light 1 - Green - rotates around X axis
        {
            float radians = lightAngle[1] * M_PI / 180.0f;
            float lightPos[4] = { 0.0f, 20.0f * sinf(radians), 20.0f * cosf(radians), 1.0f };
            glUniform3fv(pf_laUniform_sphere[1], 1, lightAmbient[1]);
            glUniform3fv(pf_ldUniform_sphere[1], 1, lightDiffuse[1]);
            glUniform3fv(pf_lsUniform_sphere[1], 1, lightSpecular[1]);
            glUniform4fv(pf_lightPositionUniform_sphere[1], 1, lightPos);
        }
        
        // Light 2 - Blue - rotates around Z axis
        {
            float radians = lightAngle[2] * M_PI / 180.0f;
            float lightPos[4] = { 20.0f * sinf(radians), 20.0f * cosf(radians), 0.0f, 1.0f };
            glUniform3fv(pf_laUniform_sphere[2], 1, lightAmbient[2]);
            glUniform3fv(pf_ldUniform_sphere[2], 1, lightDiffuse[2]);
            glUniform3fv(pf_lsUniform_sphere[2], 1, lightSpecular[2]);
            glUniform4fv(pf_lightPositionUniform_sphere[2], 1, lightPos);
        }
        ...
        // Light 1 - Green - rotates around X axis
        {
            float radians = lightAngle[1] * M_PI / 180.0f;
            float lightPos[4] = { 0.0f, 20.0f * sinf(radians), 20.0f * cosf(radians), 1.0f };
            glUniform3fv(pv_laUniform_sphere[1], 1, lightAmbient[1]);
            glUniform3fv(pv_ldUniform_sphere[1], 1, lightDiffuse[1]);
            glUniform3fv(pv_lsUniform_sphere[1], 1, lightSpecular[1]);
            glUniform4fv(pv_lightPositionUniform_sphere[1], 1, lightPos);
        }
        ...
        // Light 2 - Blue - rotates around Z axis
        {
            float radians = lightAngle[2] * M_PI / 180.0f;
            float lightPos[4] = { 20.0f * sinf(radians), 20.0f * cosf(radians), 0.0f, 1.0f };
            glUniform3fv(pv_laUniform_sphere[2], 1, lightAmbient[2]);
            glUniform3fv(pv_ldUniform_sphere[2], 1, lightDiffuse[2]);
            glUniform3fv(pv_lsUniform_sphere[2], 1, lightSpecular[2]);
            glUniform4fv(pv_lightPositionUniform_sphere[2], 1, lightPos);
        }
...
-(void)myupdate
{
    ...
    if(bLight){
        for(int i = 0; i < NO_OF_LIGHTS; i++){
            lightAngle[i] = lightAngle[i] + 2.0f;
