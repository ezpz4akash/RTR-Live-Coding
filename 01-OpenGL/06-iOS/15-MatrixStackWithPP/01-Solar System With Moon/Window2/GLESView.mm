...
@implementation GLESView
{
    ...
    // Solar system rotation angles
    GLfloat year;   // Earth's orbit around Sun
    GLfloat day;    // Earth's rotation on its axis
    GLfloat moon;   // Moon's orbit around Earth
...
-(void)display
{
    ...
    glUseProgram(shaderProgramObject);
    {
        ...
        transformationMatrixStack.push(modelViewMatrix);
        {
            // Earth: orbit around Sun (year), then translate out, then rotate on axis (day), then scale smaller
            ...
            modelViewMatrix = modelViewMatrix * rotationMatrix * translationMatrix * vmath::rotate(day, 0.0f, 1.0f, 0.0f) * scaleMatrix;

            modelViewProjectionMatrix = perspectiveProjectionMatrix * modelViewMatrix;
            
            // Blue color for Earth
            glUniform4f(colorUniform, 0.4f, 0.7f, 1.0f, 1.0f);
            glUniformMatrix4fv(mvpMatrixUniform, 1, GL_FALSE, (const float*)modelViewProjectionMatrix);

            // Draw Earth (wireframe)
            glBindVertexArray(vao_sphere);
            {
                glDrawElements(GL_LINE_STRIP, numSphereElements, GL_UNSIGNED_SHORT, 0);
            }
            glBindVertexArray(0);
            ...
            transformationMatrixStack.push(modelViewMatrix);
            {
                // Moon: orbit around Earth, scale even smaller
                scaleMatrix = vmath::scale(0.5f, 0.5f, 0.5f);
                rotationMatrix = vmath::rotate(moon, 0.0f, 1.0f, 0.0f);
                translationMatrix = vmath::translate(1.5f, 0.0f, 0.0f);
                modelViewMatrix = modelViewMatrix * rotationMatrix * translationMatrix * scaleMatrix;
...
-(void)myupdate
{
    ...
    year = year + 0.1f;   // Earth's orbit around Sun
    day = day + 0.5f;     // Earth's rotation on its axis
    moon = moon + 1.0f;   // Moon's orbit around Earth
}
