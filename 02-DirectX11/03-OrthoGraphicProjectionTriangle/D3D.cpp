...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow)
{
    ...
    hwnd = CreateWindowEx(WS_EX_APPWINDOW,
                          ...
                          TEXT("RTR 6 - Akash Musale: D3D11 Orthographic Projection Triangle"), // window name Caption Bar Name
...
HRESULT initialize(void)
{
    ...
    const float triangle_position[] = {0.0f, 50.0f, 0.0f, 50.0f, -50.0f, 0.0f, -50.0f, -50.0f, 0.0f};
...
HRESULT resize(int width, int height)
{
    ...
    if (width <= height)
    {
        orthographicProjectionMatrix = XMMatrixOrthographicOffCenterLH(-100.0f, 100.0f, (-100.0f * ((float)height / (float)width)),
                                                                       (100.0f * ((float)height / (float)width)), -100.0f, 100.0f);
    }
    else
    {
        orthographicProjectionMatrix = XMMatrixOrthographicOffCenterLH(
            (-100.0f * ((float)width / (float)height)), (100.0f * ((float)width / (float)height)), -100.0f, 100.0f, -100.0f, 100.0f);
    }
