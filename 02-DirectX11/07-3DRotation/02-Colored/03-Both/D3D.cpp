...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow)
{
    ...
    hwnd = CreateWindowEx(
            ...
            TEXT("RTR 6 - Akash Musale: D3D11 3D Rotation Colored Two2Dshapes"),  // window name Caption Bar Name
...
HRESULT initialize(void)
{
    ...
    d3d11BufferDesc.Usage = D3D11_USAGE_DEFAULT;            // gl_STATIC_DRAW 
    d3d11BufferDesc.ByteWidth = sizeof(float) * _ARRAYSIZE(pyramid_color);
    d3d11BufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
    // d3d11BufferDesc.CPUAccessFlags = 0;

    // initialize sub resource of buffer 
    ZeroMemory((void*)&d3d11SubResourceData, sizeof(D3D11_SUBRESOURCE_DATA));
    d3d11SubResourceData.pSysMem = pyramid_color;

    // now create actual buffer
    hr = gpID3D11Device->CreateBuffer(
            &d3d11BufferDesc,
            &d3d11SubResourceData,
            &gpID3D11Buffer_PyramidColorBuffer
        );

    if(FAILED(hr))
    {
        gpFile = fopen(szLogFileName, "a+");
        fprintf(gpFile, "D3D11Device::CreateBuffer() failed for pyramid color buffer\n");
        fclose(gpFile);
        return (hr);
    }
    else
    {
        gpFile = fopen(szLogFileName, "a+");
        fprintf(gpFile, "D3D11Device::CreateBuffer() Succeeded for pyramid color buffer\n");
        fclose(gpFile);
    }
    ...
    d3d11BufferDesc.Usage = D3D11_USAGE_DEFAULT;            // gl_STATIC_DRAW 
    d3d11BufferDesc.ByteWidth = sizeof(float) * _ARRAYSIZE(cube_color);
    d3d11BufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
    // d3d11BufferDesc.CPUAccessFlags = 0;

    // initialize sub resource of buffer 
    ZeroMemory((void*)&d3d11SubResourceData, sizeof(D3D11_SUBRESOURCE_DATA));
    d3d11SubResourceData.pSysMem = cube_color;

    // now create actual buffer
    hr = gpID3D11Device->CreateBuffer(
            &d3d11BufferDesc,
            &d3d11SubResourceData,
            &gpID3D11Buffer_CubeColorBuffer
        );

    if(FAILED(hr))
    {
        gpFile = fopen(szLogFileName, "a+");
        fprintf(gpFile, "D3D11Device::CreateBuffer() failed for cube color buffer\n");
        fclose(gpFile);
        return (hr);
    }
    else
    {
        gpFile = fopen(szLogFileName, "a+");
        fprintf(gpFile, "D3D11Device::CreateBuffer() Succeeded for cube color buffer\n");
        fclose(gpFile);
    }
