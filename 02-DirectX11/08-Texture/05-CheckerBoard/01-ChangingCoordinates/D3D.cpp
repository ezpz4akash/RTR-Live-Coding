...
ID3D11Texture2D *gpID3D11Texture2D_Checkerboard = NULL;
...
unsigned char checkerBoard[CHECKERBOARD_HEIGHT][CHECKERBOARD_WIDTH][4];
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow)
{
    ...
    hwnd = CreateWindowEx(
            ...
            TEXT("RTR 6 - Akash Musale: D3D11 Texture CheckerBoard Using Changing Co-ordinates"),  // window name Caption Bar Name
...
HRESULT initialize(void)
{
    ...
    // provide dynamic vertex position, texture coordinates for two quads
    // (converted from OpenGL triangle fan quads to Direct3D triangle list)
    ...
    // Texture creation is deferred to loadD3DTexture() (so we use one unified path and avoid leaks)
...
HRESULT loadD3DTexture(void)
{
    ...
    // 14 bytes file header + 40 bytes info header = 54 bytes
    const int width = CHECKERBOARD_WIDTH;
    const int height = CHECKERBOARD_HEIGHT;
    const int bpp = 4; // RGBA (32-bit)
    const int headerSize = 54;
    const int dataSize = width * height * bpp;
    const int fileSize = headerSize + dataSize;

    BYTE* pBmpBuffer = new BYTE[fileSize];
    // BITMAPFILEHEADER
    pBmpBuffer[0] = 'B';
    pBmpBuffer[1] = 'M';
    *(int*)&pBmpBuffer[2] = fileSize;       // bfSize
    *(int*)&pBmpBuffer[6] = 0;              // bfReserved1 + bfReserved2
    *(int*)&pBmpBuffer[10] = 54;            // bfOffBits

    // BITMAPINFOHEADER
    *(int*)&pBmpBuffer[14] = 40;            // biSize
    *(int*)&pBmpBuffer[18] = width;         // biWidth
    *(int*)&pBmpBuffer[22] = height;        // biHeight
    *(short*)&pBmpBuffer[26] = 1;           // biPlanes
    *(short*)&pBmpBuffer[28] = 32;          // biBitCount
    *(int*)&pBmpBuffer[30] = 0;             // biCompression (BI_RGB)
    *(int*)&pBmpBuffer[34] = 0;             // biSizeImage
    *(int*)&pBmpBuffer[38] = 0;             // biXPelsPerMeter
    *(int*)&pBmpBuffer[42] = 0;             // biYPelsPerMeter
    *(int*)&pBmpBuffer[46] = 0;             // biClrUsed
    *(int*)&pBmpBuffer[50] = 0;             // biClrImportant

    // Copy pixel data
    memcpy(&pBmpBuffer[54], checkerBoard, dataSize);

    // Create texture from memory using DirectXTK
    hr = DirectX::CreateWICTextureFromMemory(gpID3D11Device, pBmpBuffer, fileSize, (ID3D11Resource**)&gpID3D11Texture2D_Checkerboard, &gpID3D11ShaderResourceView_Texture);
    delete[] pBmpBuffer;

    if (FAILED(hr))
    {
        gpFile = fopen(szLogFileName, "a+");
        fprintf(gpFile, "DirectX::CreateWICTextureFromMemory() failed in loadD3DTexture()\n", hr);
        fclose(gpFile);
        return hr;
    }

    // create sampler state for texture sampling in shader
    D3D11_SAMPLER_DESC d3d11SamplerDesc;
    ZeroMemory((void *)&d3d11SamplerDesc, sizeof(D3D11_SAMPLER_DESC));
    d3d11SamplerDesc.Filter = D3D11_FILTER_MIN_MAG_MIP_POINT; // Nearest neighbor
    ...
    {
        gpFile = fopen(szLogFileName, "a+");
        fprintf(gpFile, "ID3D11Device::CreateSamplerState() failed in loadD3dTexture()\n");
        fclose(gpFile);
        return hr;
    }
...
void display(void)
{
    ...
    // Set both vertex buffers (position and texcoord)
...
void uninitialize(void)
{
    ...
    if (gpID3D11Texture2D_Checkerboard)
    {
        gpID3D11Texture2D_Checkerboard->Release();
        gpID3D11Texture2D_Checkerboard = NULL;
    }
