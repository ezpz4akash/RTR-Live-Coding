...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow)
{
    ...
    hwnd = CreateWindowEx(WS_EX_APPWINDOW,
                          ...
                          TEXT("RTR 6 - Akash Musale: D3D11 2D Rotation Triangle Color"),       // window name Caption Bar Name
...
HRESULT initialize(void)
{
    ...
    {
        gpFile = fopen(szLogFileName, "a+");
        fprintf(gpFile, "D3D11Device::CreateBuffer() failed for color buffer\n");
        fclose(gpFile);
        return (hr);
    }
    else
    {
        gpFile = fopen(szLogFileName, "a+");
        fprintf(gpFile, "D3D11Device::CreateBuffer() Succeeded for color buffer\n");
        fclose(gpFile);
    }
