...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow)
{
    ...
    hwnd = CreateWindowEx(WS_EX_APPWINDOW,
                          ...
                          TEXT("RTR 6 - Akash Musale: D3D11 Two 2D Shapes B/W Rotation"),       // window name Caption Bar Name
...
HRESULT initialize(void)
{
    ...
    {
        gpFile = fopen(szLogFileName, "a+");
        fprintf(gpFile, "D3D11Device::CreateBuffer() failed for triangle\n");
        fclose(gpFile);
        return (hr);
    }
    else
    {
        gpFile = fopen(szLogFileName, "a+");
        fprintf(gpFile, "D3D11Device::CreateBuffer() Succeeded for triangle\n");
        fclose(gpFile);
    }
    ...
    {
        gpFile = fopen(szLogFileName, "a+");
        fprintf(gpFile, "D3D11Device::CreateBuffer() failed for rectangle\n");
        fclose(gpFile);
        return (hr);
    }
    else
    {
        gpFile = fopen(szLogFileName, "a+");
        fprintf(gpFile, "D3D11Device::CreateBuffer() Succeeded for rectangle\n");
        fclose(gpFile);
    }
...
HRESULT resize(int width, int height)
{
    ...
    perspectiveProjectionMatrix = XMMatrixPerspectiveFovLH(XMConvertToRadians(45.0f), (float)width / (float)height, 0.1f, 1000.0f);
...
{
    // code
    angleTriangle = angleTriangle + 0.03f;
    if (angleTriangle >= 360.0f)
    {
        angleTriangle = angleTriangle - 360.0f;
    }

    angleRectangle = angleRectangle - 0.03f;
