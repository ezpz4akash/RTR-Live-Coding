...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow)
{
    ...
    hwnd = CreateWindowEx(
            ...
            TEXT("RTR 6 - Akash Musale: D3D11 Three Moving Lights On Static Sphere Per Fragment"),  // window name Caption Bar Name
...
HRESULT initialize(void)
{
    ...
    "   float3 lightDirection0 : TEXCOORD1;" \
    ...
    "   output.viewerVector = normalize(-(float3)eyeCoordinates);" \
    "   if(lKeyPressed == 1)" \
    "   {" \
    "       output.lightDirection0 = (float3)lightPosition[0] - (float3)eyeCoordinates;" \
    "       output.lightDirection1 = (float3)lightPosition[1] - (float3)eyeCoordinates;" \
    "       output.lightDirection2 = (float3)lightPosition[2] - (float3)eyeCoordinates;" \
    "   }" \
    "   else" \
    "   {" \
    "       output.lightDirection0 = float3(0.0f, 0.0f, 0.0f);" \
    "       output.lightDirection1 = float3(0.0f, 0.0f, 0.0f);" \
    "       output.lightDirection2 = float3(0.0f, 0.0f, 0.0f);" \
    "   }" \
    ...
    "   if(lKeyPressed == 1)" \
    "   {" \
    "       float3 normalisedTransformedNormal = normalize(input.transformedNormal);" \
    "       float3 normalisedViewerVector = normalize(input.viewerVector);" \
    "       float3 lightDir0 = normalize(input.lightDirection0);" \
    "       float3 lightDir1 = normalize(input.lightDirection1);" \
    "       float3 lightDir2 = normalize(input.lightDirection2);" \
    "       float3 lightDirections[3] = {lightDir0, lightDir1, lightDir2};" \
    "       for(int i = 0; i < 3; i++)" \
    "       {" \
    "           float3 ambientLight = (float3)La[i] * (float3)Ka;" \
    "           float3 diffuseLight = (float3)Ld[i] * (float3)Kd * max(dot(lightDirections[i], normalisedTransformedNormal), 0.0f);" \
    "           float3 reflectionVector = reflect(-lightDirections[i], normalisedTransformedNormal);" \
    "           float3 specularLight = (float3)Ls[i] * (float3)Ks * pow(max(dot(reflectionVector, normalisedViewerVector), 0.0f), materialShininess);" \
