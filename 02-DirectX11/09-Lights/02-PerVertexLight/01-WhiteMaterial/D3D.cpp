...
HRESULT initialize(void)
{
    ...
    const char *vertexShaderSourceCode =
        ...
        "       float3 diffuseLight = (float3)Ld * (float3)Kd * max(dot(lightDirection, transformedNormal), 0.0f);"
        "       float3 reflectionVector = reflect(-lightDirection, transformedNormal);"
        "       float3 viewerVector = normalize(- (float3)eyeCoordinates);"
        "       float3 specularLight = (float3)Ls * (float3)Ks * pow(max(dot(reflectionVector, viewerVector), 0.0f), materialShininess);"
        "       float3 phongColor = ambientLight + diffuseLight + specularLight;"
