...
ID3D11GeometryShader *gpID3D11GeometryShader = NULL; // for geometry shader
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow)
{
    ...
    hwnd = CreateWindowEx(WS_EX_APPWINDOW,
                          ...
                          TEXT("RTR 6 - Akash Musale: D3D11 Geometry Shader"),                  // window name Caption Bar Name
...
HRESULT initialize(void)
{
    ...
    const char *geometryShaderSourceCode =
        ...
        "       geometry_output.position = mul(worldViewProjectionMatrix, (input[vertex].position + float4(0.0, 1.0, 0.0, 0.0)));"
        "       triangleStream.Append(geometry_output);"

        "       geometry_output.position = mul(worldViewProjectionMatrix, (input[vertex].position + float4(1.0, -1.0, 0.0, 0.0)));"
        "       triangleStream.Append(geometry_output);"
    ...
    hr = D3DCompile(geometryShaderSourceCode, lstrlenA(geometryShaderSourceCode) + 1,
                    ...
                    0,                             // Flags
                    0,                             // Effect Constant
                    &pID3DBlob_geometryShaderCode, // compiled code thevnya sathi
                    &pID3DBlob_error               // Error for vertex shader
    );
    ...
    {
        gpFile = fopen(szLogFileName, "a+");
        fprintf(gpFile, "D3DCompile() succeeded for geometry shader\n");
        fclose(gpFile);
    }
    ...
    {
        gpFile = fopen(szLogFileName, "a+");
        fprintf(gpFile, "D3D11Device::CreateGeometryShader() failed\n");
        fclose(gpFile);
        return (hr);
    }
    else
    {
        gpFile = fopen(szLogFileName, "a+");
        fprintf(gpFile, "D3D11Device::CreateGeometryShader() Succeeded\n");
        fclose(gpFile);
    }
    ...
    /////////////////////////////////////// Pixel Shader ///////////////////////////////////////
    const char *pixelShaderSourceCode = "float4 main(void) : SV_TARGET"
                                        "{"
    ...
    const float triangle_position[] = {0.0f, 1.0f, 0.0f, 1.0f, -1.0f, 0.0f, -1.0f, -1.0f, 0.0f};
