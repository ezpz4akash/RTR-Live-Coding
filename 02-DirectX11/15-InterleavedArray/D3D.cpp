...
struct CBUFFER
{
    ...
    XMVECTOR La; // Light Ambient
    XMVECTOR Ld; // Light Diffuse
    XMVECTOR Ls; // Light Specular
    XMVECTOR LightPosition;
    XMVECTOR Ka; // Material Ambient
    XMVECTOR Kd; // Material Diffuse
    XMVECTOR Ks; // Material Specular
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow)
{
    ...
    hwnd = CreateWindowEx(WS_EX_APPWINDOW,
                          ...
                          TEXT("RTR 6 - Akash Musale: D3D11 InterleavedArray"),                 // window name Caption Bar Name
...
HRESULT initialize(void)
{
    ...
    const char *vertexShaderSourceCode =
        ...
        "       float3 normalisedTransformedNormal = normalize(mul(normalMatrix, (float3)norm));"
        "       float3 normalisedLightDirection = normalize((float3)(lightPosition - eyeCoordinates));"
        "       float3 normalisedViewerVector = normalize(-eyeCoordinates.xyz);"
        "       float3 ambientLight = La.xyz * Ka.xyz;"
        "       float3 diffuseLight = Ld.xyz * Kd.xyz * max(dot(normalisedLightDirection, normalisedTransformedNormal), 0.0);"
        "       float3 reflectionVector = reflect(-normalisedLightDirection, normalisedTransformedNormal);"
        "       float3 specularLight = Ls.xyz * Ks.xyz * pow(max(dot(reflectionVector, normalisedViewerVector), 0.0), materialShininess);"
        "       vertex_output.phong_ads_light = ambientLight + diffuseLight + specularLight;"
        "   }"
        "   else"
        "   {"
        "       vertex_output.phong_ads_light = float3(1.0f, 1.0f, 1.0f);"
        "   }"
        ""
    ...
    const char *pixelShaderSourceCode = "cbuffer constantBuffer"
                                        ...
                                        "{"
                                        "   float3 phong_ads_light = input.phong_ads_light;"
                                        "   float4 texColor = myTexture2D.Sample(mySamplerState, input.texcoord);"
                                        "   float4 color;"
                                        ""
                                        "   if(tKeyPressed == 1)"
                                        "   {"
                                        "       color = texColor * input.color * float4(phong_ads_light, 1.0f);"
                                        "   }"
                                        "   else"
                                        "   {"
                                        "       color = input.color * float4(phong_ads_light, 1.0f);"
                                        "   }"
                                        "   return color;"
                                        "}";
    ...
    // texcoord - 2 floats at offset 36
    ...
    // set constant buffer for pixel shader as well (for tKeyPressed flag)
...
void display(void)
{
    ...
    // Set vertex buffer with interleaved data
    UINT stride = sizeof(float) * 11; // position(3) + color(3) + normal(3) + texcoord(2)
    UINT offset = 0;                  // color data starts
    gpID3D11DeviceContext->IASetVertexBuffers(0, 1, &gpID3D11Buffer_PositionBuffer, &stride, &offset);

    stride = sizeof(float) * 11; // position(3) + color(3) + normal(3) + texcoord(2)
    offset = sizeof(float) * 3;  // color data starts
    gpID3D11DeviceContext->IASetVertexBuffers(1, 1, &gpID3D11Buffer_PositionBuffer, &stride, &offset);

    stride = sizeof(float) * 11; // position(3) + color(3) + normal(3) + texcoord(2)
    offset = sizeof(float) * 6;  // normal data starts
    gpID3D11DeviceContext->IASetVertexBuffers(2, 1, &gpID3D11Buffer_PositionBuffer, &stride, &offset);

    stride = sizeof(float) * 11; // position(3) + color(3) + normal(3) + texcoord(2)
    offset = sizeof(float) * 9;  // texcord data starts
    gpID3D11DeviceContext->IASetVertexBuffers(3, 1, &gpID3D11Buffer_PositionBuffer, &stride, &offset);

    // Set primitive Topology
    gpID3D11DeviceContext->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST);

    // Set shader resource view to pipeline for texture
    ...
    if (bLight == TRUE)
    {
        ...
        constantBuffer.MaterialShininess = 128.0f;
    }
    else
    {
